...
export function DisclosureCard({
  ...
  return (
    <article
      className={`flex h-full flex-col ${mobileCardPadding} ${mobileCardShell} rounded-xl border border-[#E3DED4] bg-white ${shadows.card}`}
    >
      <div className="min-w-0 flex-1">
        <p className="text-[10px] font-bold text-[#B9975B]">
          {publicDisclosureCategoryLabels[entry.category]}
        </p>
        <h3 className={`mt-2 ${mobileCardTitle}`}>{entry.title}</h3>
        <p className="mt-1 line-clamp-1 break-keep text-xs font-medium text-[#4A5565]">
          {scopeLabel}
        </p>
        ...
        <p className={`mt-3 ${mobileCardDescription}`}>
          {entry.description}
        </p>
        <p className="mt-2 line-clamp-2 break-keep text-xs leading-relaxed text-[#4A5565]">
      ...
      <div className={mobileCardActions}>
        ...
        <div className={mobileCardActionsTight}>
          {entry.insurerName ? (
            <Link
              className={`${buttons.base} ${buttons.outline} w-full px-3 text-xs sm:min-w-[8rem] sm:flex-1`}
              href={directoryHref}
            >
              관련 보험사 보기
            </Link>
