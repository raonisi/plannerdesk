...
export async function buildAdminDashboardSnapshot(): Promise<AdminDashboardSnapshot> {
  ...
  const features: AdminDashboardFeature[] = [
    ...
    knowledgeFeature(knowledgeProbe),
    {
      ...
      title: "공시·약관 링크 관리",
      description:
        "공식 공시실, 약관, 보험사 공식 링크를 검수 기준에 따라 관리합니다.",
      href: "/admin/disclosure-links",
      availability: "active_with_warning",
      statusBadge: "조회 모드",
      lastCheckLabel: "정적 데이터 · DisclosureLink DB 예정",
      nextAction:
        "현재 lib/content 정적 목록을 점검합니다. 저장·일괄 변경은 DB PR 이후 제공됩니다.",
      buttonLabel: "확인하기",
      buttonEnabled: true,
    },
    {
  ...
  const bulkWorkflows: AdminBulkWorkflow[] = [
    ...
    },
    {
      ...
      statusBadge: "준비 중",
      nextAction: "DisclosureLink DB 모델 도입 후 일괄 저장이 연결됩니다.",
      buttonLabel: "준비 중",
      buttonEnabled: false,
    },
    {
