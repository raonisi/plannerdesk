...
25. PR-38 ClaimDocument admin CRUD (protected `/admin/claim-documents` create/update + publish-toggle, slug uniqueness + prohibited-phrase deny-list + draft-publish guard; no hard delete; no public DB read) (Current)
    - HIGH-risk admin write change. Antigravity review required.
    - See [docs/CLAIM_DOCUMENT_MODEL_PLAN.md ?M](file:///c:/work/plannerdesk/plannerdesk-main/docs/CLAIM_DOCUMENT_MODEL_PLAN.md) for shipped scope.
...
PR-38 ships the protected admin CRUD surface for `ClaimDocument` records under `/admin/claim-documents` (list, create, edit, publish/unpublish toggle). It reuses the existing RBAC helpers (`canAccessAdmin`, `canManageContent`, `canPublishContent`) via a module-local `app/admin/claim-documents/access.ts` wrapper that mirrors the Insurer admin's pattern. The server actions enforce: a strict slug shape (`^[a-z0-9]+(?:-[a-z0-9]+)*$`, ≤ 80 chars) checked client-side and re-verified server-side, a prohibited-phrase deny-list scan of every public-facing field (Section I), hard text caps on each free-text column, optional URL validation, the canonical draft-publish guard from PR-33, and a re-read of `verificationStatus` inside `setClaimDocumentPublished` so a stale list page cannot smuggle a draft to the public surface. Optional `Insurer` linkage is validated against the database on every write, but the FK relationship from PR-37 (`ON DELETE SET NULL`) continues to protect against orphaning at the DB level. **Hard delete is deliberately not implemented**: there is no `prisma.claimDocument.delete` call anywhere in the diff and no delete affordance on the list page. Outdated records must be unpublished or downgraded to `needs_review` per `docs/CLAIM_DOCUMENT_MODEL_PLAN.md` Section G #7. The form and list pages surface two verbatim Korean notices required by the PR-38 task spec (the "안내 성격 한정" and "민감정보 입력 금지" notices) and the form additionally lists the prohibited phrases so operators see the deny-list before they save. PR-38 does **not** add public DB read, API routes, file upload, OCR, AI claim judgment, customer claim submission, or any field that implies payout judgment or stores customer/medical data. `prisma/schema.prisma`, `prisma/migrations/*`, `auth.ts`, and `middleware.ts` are all untouched. See [docs/CLAIM_DOCUMENT_MODEL_PLAN.md Section M](file:///c:/work/plannerdesk/plannerdesk-main/docs/CLAIM_DOCUMENT_MODEL_PLAN.md) for the shipped scope, the canonical Korean copy, the validation rules, and the explicit list of fields that must remain absent.

PR-39 converts the public `/claim-documents` route from static mock data to a database-backed dynamic render of published and verified `ClaimDocument` records. A new server-side public read helper `getPublicClaimDocuments` defined in `lib/public/claim-documents.ts` queries records with `isPublished: true` and a verification status in `[verified, needs_review]` (centralizing the canonical visibility status filter imported from `lib/public/insurers.ts`). It projects only public-safe fields, excluding internal admin metadata (`createdById`, `updatedById`, etc.), and handles database errors gracefully. The client Explorer component `ClaimDocumentExplorer` uses the `PublicClaimDocument` type, translates `ClaimDocumentCategory` values to Korean labels, filters by insurer, and enforces the compliance rules. Verification badges are displayed, and missing links/fields fall back to safe Korean placeholders. Public access remains completely login-free. PR-39 does not implement public claim submission, file uploads, OCR, AI payout estimation, or medical/customer data storage, and the database schema is unchanged.
