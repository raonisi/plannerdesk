...
PR-32 adds a local-only favorites layer on top of the PR-31 card. The `hooks/useFavorites.ts` hook reads from and writes to `localStorage` under a clearly namespaced key (`plannerdesk:favoriteInsurers`), stores only insurer ids (no phone numbers, URLs, account data, or customer data), and degrades silently when storage is unavailable. The favorites view applies on top of the already-published list, so unpublished records cannot leak. The next implementation work after PR-32 is PR-33 (Verification/publish workflow polish).
