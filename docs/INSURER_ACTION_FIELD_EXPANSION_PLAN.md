...
PR-30 wires the public `/directory` page to read published `Insurer` records from the database through `lib/public/insurers.ts`, projecting only the public-safe fields enumerated in Section E and applying the data-governance fallback copy from Section F. The next implementation work after PR-30 is PR-31 (Public insurer action card UI polish).
