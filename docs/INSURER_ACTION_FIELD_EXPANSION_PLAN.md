...
PR-31 polishes the public action card UI on top of the PR-30 read path. It introduces a reusable `components/directory/insurer-action-card.tsx` component, centralizes display formatting in `lib/directory/formatting.ts`, and reshapes each card into the grouped sections from Section G (Access / Support / Claim / Policy / Payment) without changing visibility, schema, or admin write behavior. The next implementation work after PR-31 is PR-32 (Favorites localStorage MVP).
